...
Each session deck control group provides “Return selected to this deck top” and “Return selected to this deck bottom” for the current public-table selection. Buttons are disabled until one or more eligible table cards are selected. The request carries the selected card ids and exact expected card-version map; the service validates all versions, locations, visibility, locks, and the actor's `card.manage` and `deck.manage` capabilities atomically. Selection order determines the returned group order. This control does not reveal card identity or source-deck association and the durable public event omits card ids.
