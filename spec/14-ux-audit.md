...
Source commit `bfb1264` implements the compact/top-aligned shell, hides the marketing banner while a table is open and names the table region accessibly, widens the table workspace, adds accessible title/status/role filtering to the caller's own table list, and darkens the empty-hand text while retaining the light-on-green table message. Contract suite passes 34/34, TypeScript typecheck, Vite build, and diff check. A local preview server started, but the available computer-use inventory returned no browser/app surfaces, so screenshot and interaction review could not be completed. Recheck at desktop and narrow viewport after deployment; keep this UX result provisional.
...
## Table viewport zoom response — 2026-09-23

The `spec/07-table-interaction.md` contract now defines local 50%–150% zoom, native scroll/keyboard/touch panning, centered zoom, coordinate conversion for dragged cards and piles, and viewport preservation across refreshed snapshots. Source adds bounded Zoom out/Zoom in/Reset zoom controls, an announced zoom level, a focusable tabletop region, and touch-sized mobile controls. Zoom remains browser-only and is not sent as session state; drag movement is converted from screen deltas to the original board coordinate scale.

The source passes 35/35 Python contract tests, TypeScript typecheck, Vite production build, and `git diff --check`. Visual and interaction acceptance is still open: the available computer-use inventory returned no apps or browsers, so desktop/narrow viewport screenshots, touch panning, centered scaling and drag-at-zoom behavior were not observed. Recheck those on the released source. Card artwork and review of card backs against real assets also remain open, as do the previously recorded role/privacy, concurrency and recovery acceptance items.

## Card-front artwork response — 2026-09-23

After the security contract in `spec/03-identity-visibility.md` and `spec/06-assets-templates.md` (`dc0e681`), source adds same-origin card-front delivery bound to a session/card pair. The PHP service resolves the asset only after checking active membership and current visibility; the client places an image in visible face-up/own-hand/own-private cards, while hidden foreign-private cards receive no image element or asset identifier. Safe card labels remain the accessible names and the images are decorative.

Contract checks pass 36/36, TypeScript and Vite production builds pass, and diff check is clean. No PHP/PostgreSQL runtime or browser surface is available here. The visual treatment, image request on an authorized card, stale-route 404 after hide/removal, and denial to another participant require live two-account acceptance. Default/custom card backs still need a separate review against real assets.

## Lock-control UX response — 2026-09-23

The table surface now exposes selected-card, pile, per-deck, per-zone, and whole-table lock controls when the current participant has the matching capabilities. Locked objects remain visible; the projection reports only lock state and whether the current viewer owns the lock. A participant facing a foreign whole-table lock sees a read-only notice while refresh, view zoom/pan, authorized unlock, and host reset recovery remain available. Locked foreign cards can be selected for an authorized unlock, while their move/flip and bulk controls are disabled. Zone locks can be released from an always-available zone panel, including when the table is active, and locked zones cannot be edited by another participant.

The labels, disabled states, and lock-owner flows are source-checked by the 37-test contract suite and TypeScript build. No authenticated browser review was possible. Validate each scope with a lock holder and a second participant, including keyboard access, overlapping zones, hidden cards, reconnect, and reset, before treating this UX change as accepted.

## Protected card-back response — 2026-09-23

Face-down public and foreign-private table cards can now render their per-card back asset, falling back to the immutable template-version default and then to the existing generic CSS back. The snapshot exposes only a `has_back_art` boolean; a session/card-bound route rechecks membership, table location and non-face-up state and returns private `no-store` bytes. The image is decorative, so the existing safe card label remains the accessible name. No asset ID or storage path is projected.

Commit `b43660f` implements this behavior after the contract in `spec/03-identity-visibility.md` and `spec/06-assets-templates.md`. The full source-contract suite passes 38/38; TypeScript typecheck, Vite build and diff check pass. Browser review and PHP/PostgreSQL route verification remain open; test card-specific/default/no-back cards, foreign-private backs, face-up transitions, moved/removed cards and non-member or removed-member requests against the deployed release.

## Current UX audit checkpoint — 2026-09-23

The source now contains the compact home shell and resumable-table filtering, improved empty-hand contrast, a wider table workspace, bounded zoom and pan, accessible card/pile/participant/zone controls, safe recent-action descriptions, and protected front/back artwork. The current source checks pass (38 contract tests, TypeScript typecheck, Vite build, and `git diff --check`), but these are not visual or assistive-technology acceptance.

No browser was available in this session: the computer-use inventory returned no apps or browsers, and opening an in-app browser failed because the surface was unavailable. I did not log in or interact with production. Read-only HTTP probes returned 200 for `/` and `/health`; the page serves old CSS/JS assets (`index-C4nun6z5.css`, `index-BIUiNAMK.js`) while the active deployer release is older than source. The prior live UX record on that old release reported successful sign-in and keyboard flip/restore, a reconnect status that remained closed/retrying after mutations, a crowded home/table layout, and low empty-hand contrast. Source changes address several of those visual findings, but the live page cannot demonstrate them until it serves the current source.

The UX result remains **provisional**. The next UX pass should use the exact accepted release and a browser with desktop and narrow/touch viewports. Verify sign-in/recovery states, table-list scanning, keyboard and screen-reader operation, pan/zoom and drag coordinates, populated card/pile flows, custom front/back artwork and authorization errors, and two-participant privacy/reconnect behavior. Template authoring is also a product-flow gap: users can select owned templates, but the frontend has no upload/create/version editor. Do not treat the source build as visual acceptance or add broader UI beyond findings confirmed in that pass.

## Template authoring and host-freeze response — 2026-09-23

Source commit `5ba394b`, following committed contracts in `spec/06-assets-templates.md` and `spec/23-host-interaction-freeze.md`, adds an authenticated template editor with owned image previews/uploads, front/back/default-back choices, quantity and safe labels, accessible move-up/move-down ordering, and separate create-template/add-version actions. The table now shows a frozen-state notice and host Freeze/Unfreeze controls while suppressing gameplay and zone-edit affordances during a freeze; participant and recovery controls remain available as specified.

Source checks pass (40 contract tests, TypeScript, Vite build and diff check), but the browser service was unavailable and the deployed app was not opened. The editor, frozen banner, screen-reader order, keyboard reorder/upload flow, card previews, narrow-screen behavior and disabled/read-only controls have not been visually reviewed. UX acceptance remains provisional and must be completed against the exact deployed release with at least two participants.

## Public-release UX evidence refresh — 2026-09-23

The current repository build generates `index-7MqU83IC.css` and `index-DGwfojO6.js`. Fresh HTTP retrieval of those production assets matched the local build byte-for-byte (9,581 CSS bytes and 61,504 JS bytes), and `/`, `/health`, and `/login` returned 200. This supersedes the earlier checkpoint's statement that the public page still served old bundle names. Unauthenticated `/api/assets` returned 401 and a guessed protected-asset URL returned 404.

The computer-use inventory still has no browser or app surfaces. No sign-in, visual inspection, keyboard/screen-reader pass, responsive/touch test, or two-participant interaction was possible. Earlier layout and reconnect observations were made on a historical release and do not establish how the current frontend looks or behaves. The UX result remains **provisional** until the current deployed interface is reviewed at desktop and narrow/touch sizes, including account recovery, table/template flows, zoom/pan/drag, protected card art, accessibility, and realtime with a second participant.

## Audit-only UX recheck after requested resume — 2026-09-23

The browser connector still reports no available browser or app, so the requested sign-in and visual review could not be carried out. Fresh HTTP checks only reconfirmed 200 responses for `/`, `/health`, and `/login` and the expected current frontend asset names; they do not show rendered layout or prove interactive behavior. The deployer reports all three production services active but unhealthy, with active source `cd60963` and desired source `456b49b`; this prevents claiming that the service set is an accepted release even though public frontend assets match the recorded local build.

No new UX finding was established and no code, account, session, or email state changed. The audit remains **provisional**. The next review requires browser access and exact-release provenance, then should inspect the actual login/recovery journey, home and template editor, table setup/resume, populated card and pile controls, participant/freeze/lock states, front/back artwork, realtime after mutations, keyboard and screen-reader cues, and narrow/touch viewport pan/zoom and drag behavior. Record observed failures separately from source-level expectations; do not add polish based only on assumptions.

## Exact-release UX verification checkpoint — 2026-09-23
