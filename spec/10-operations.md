...
The PHP-FPM release entrypoint runs the committed checksum-verified migration runner before starting FPM. Migrations use explicit PostgreSQL transactions; an execution or checksum failure stops startup so the deployer's health check cannot accept a release against a partial schema. Previously active PHP, Node and mail releases must remain compatible with each expand-only migration while services are activated serially. An already-applied migration is a no-op after its checksum is verified, so ordinary restarts do not repeat schema changes. This keeps migration execution in the reviewed release lifecycle and records output through its normal service logs.
