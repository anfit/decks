...
The current Windows system Node `20.20.0` and PostgreSQL client `14.5` are below the production runtime contract; use Node `>=22.22 <25` for application installs and runtime checks. PHP is not installed in the Windows shell, and there is no WSL distribution configured, so PHP/FPM and PostgreSQL 18 integration requires the production host or an equivalent pinned Linux environment. The verified OVH production host provides Node `24.21.0`/npm `11.19.0`, PHP `8.5.4` with PHP-FPM 8.5, and PostgreSQL `18.6`. The package engine, lockfile and deployer preflight enforce the Node and PostgreSQL bounds so production does not silently use an unsupported runtime. Production prerequisites must assert PHP, PostgreSQL and Node versions actually installed on `prod`; they must not be guessed from this workstation.
