...
1. Restore the trusted runtime environment, refresh all three OVH manifests against exact source `1dc3085`, and deploy web/realtime/mail only after the required secret preflight succeeds. Run remote PHP lint, check manifests/provenance and health, and verify repeat plans are a no-op. Do not inspect or copy values from `tmp/.env`.
