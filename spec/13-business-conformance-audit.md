...
Spec-first commit `dc0e681` extends `spec/03-identity-visibility.md` and `spec/06-assets-templates.md` with per-request viewer authorization for protected card-front delivery. Source now resolves the front asset only from the bound session/card after confirming active membership and the card's current face/location/owner. The frontend displays artwork only for public face-up cards, the current player's hand, and a private table card whose authorized projection contains its definition handle. The DOM route contains session/card handles only; no asset ID or front URL enters a snapshot. The old owner-only preview route remains independent, and protected responses use private no-store caching.

Validation passes 36/36 Python contract checks, TypeScript typecheck, Vite production build and `git diff --check`. PHP lint/database execution and browser checks are not available in this environment, so the query, nonmember/removed-member rejection, stale URL denial, protected-file delivery and rendered image behavior remain unverified at runtime. Card-front art is now source-complete but must pass exact-release two-account acceptance before business conformance can be upgraded.

## Business-spec §20 lock-scope response — 2026-09-23

The revision-1 business specification explicitly supports individual-card, pile, zone, deck, and whole-table administrative locks. `spec/22-resource-lock-scopes.md` and the updated capability matrix in `spec/19-capability-policy.md` define the required grants, safe per-viewer lock indicators, expected versions/revisions, overlapping-zone checks, holder behavior, and reset recovery. Migration `006_resource_lock_scopes.sql` adds durable owners for deck, zone, and whole-table locks while preserving legacy zone lock state.

The current source routes deck, zone, and table lock actions through the transactional action service. Lock checks run before idempotent replay and enforce deck targets, card/pile source positions, and requested table positions; a whole-table lock also blocks new joins. Authorized host reset clears every administrative lock scope. Card and pile lock controls now support authorized foreign unlocks with fresh object versions. Snapshots expose only `locked` and the viewer-relative `locked_by_current` boolean, never lock-owner IDs.

The source contract suite passes 37/37, TypeScript typecheck and Vite production build pass, and `git diff --check` is clean. PHP lint, PostgreSQL migration/action execution, overlapping-zone transactions, and two-client lock acceptance remain unverified because no local PHP runtime or browser surface is available. Include card, pile, deck, zone, whole-table, foreign-owner, holder, stale-version, join-denial, and reset cases in exact-release acceptance. The business result remains **partial** until runtime work, deployment, remaining scope decisions, and operational gates are evidenced.

## Protected card-back rendering response — 2026-09-23

The business template contract requires a default card back and allows per-card backs. Source now renders a per-card back when defined, otherwise the immutable template-version default, for face-down public and foreign-private cards on the table. The projection exposes only whether back art exists; the new protected route resolves by session/card and rechecks membership and current non-face-up table state. Cards without back art retain the generic CSS back. This closes the source rendering gap only; current production, custom/default fallback, privacy and stale-route behavior still require exact-release browser acceptance.

## Current business audit checkpoint — 2026-09-23

This is a read-only follow-up against source `b43660fb08e7b56263f0a9fb5fa86c594218d282`, the committed specs, and the pinned revision-1 business baseline in `spec/00-scope.md`. The shared Special URL was not accessible to the web reader in this session and the computer-use environment had no browser; this checkpoint therefore does not claim a fresh browser read or authenticated acceptance. The prior audit's direct inspection of revision 1 and its hash-matched baseline remain the business reference.

The source now includes the previously open user-facing multi-deck setup, participant administration, generic zone editor, card/pile/deck/zone/table lock scopes, card-front and protected card-back rendering, recent safe action history, hand controls, and the previously audited deck/pile/group actions. The current contract checks pass 38/38; TypeScript typecheck, Vite production build, and `git diff --check` pass. These checks do not execute PHP/PostgreSQL transactions, migrations, or protected-image authorization.

Two source-scope gaps remain visible in a static product-path review. Template and asset creation/versioning are available through authenticated APIs but the normal frontend only reads the caller's template list, so a user has no in-app workflow to upload artwork and author a reusable deck template. The business role section also calls for host freeze/unfreeze; no corresponding frozen state or action is present. Add these only if the user-facing audit confirms they remain in the intended MVP, with a small committed behavior spec before implementation. Creation-time permission selection and access modes beyond scoped invitation links remain policy questions; the current setup exposes permissions after creation and keeps access invite-only.

Production is still on active release `a70c82c42a9503a6`, source `4e51b2db413f52421e7637d7863913d8c84aa69e`, while the current desired source is `b43660f`. Read-only deployer status reports all three Decks services active but unhealthy because active provenance/manifests are stale or missing. Public `/` and `/health` return HTTP 200, but `/` references the older `index-C4nun6z5.css` and `index-BIUiNAMK.js` assets. No deployment, login attempt, table mutation, or mail send occurred in this checkpoint. The prior authenticated browser audit on this same old source recorded successful sign-in and a durable keyboard face flip/restore, followed by `Realtime connection closed; retrying`; that reconnect finding remains unclosed.

The business result remains **partial and unaccepted**. The source has broad coverage of the revision-1 minimum workflow, but production is stale, realtime recovery has a prior failing live observation, template authoring and host freeze are unresolved source gaps, and no current exact-release multi-participant runtime check was possible. Keep the project at audit/roadmap scope until a separately authorized release and browser-acceptance task.

## Template authoring and host-freeze response — 2026-09-23

Spec-first commit `397370e` adds owner-scoped asset/template authoring to `spec/06-assets-templates.md` and the durable host interaction freeze contract in `spec/23-host-interaction-freeze.md`. Source commit `5ba394b` adds explicit ownership grants for deduplicated assets, safe owned-asset listing and preview, front/back/default-back selection, ordered definition editing, and immutable template creation/versioning in the authenticated home. It also adds durable `interaction_frozen`, exact-revision host freeze/unfreeze actions, a safe boolean snapshot field, atomic denial of gameplay mutations, documented host recovery actions, and reset clearing.

The 40 Python contract checks, TypeScript typecheck, Vite production build and diff checks pass. PHP syntax, migration execution, image byte delivery, database transaction behavior and action/idempotency behavior are not verified at runtime. The computer-use surface again reported no apps or browsers, so no login or browser acceptance occurred. This closes the two known source implementation gaps only; the result remains partial until deployment provenance, runtime checks, and exact-release multi-user acceptance pass.

## Public-release and source follow-up — 2026-09-23

The application source remains at `cd6096393788643f42e09e4f9013cbc7c995cfc1`; documentation-only audit refresh is commit `86dba9a`. A fresh Vite build produced `index-7MqU83IC.css` and `index-DGwfojO6.js`; downloading both public production assets returned byte-for-byte matches to those local build files (9,581 CSS bytes and 61,504 JS bytes). Public `/` and `/health` returned 200, `/login` returned 200, unauthenticated `/api/assets` returned 401 `authentication_required`, and a guessed protected-asset URL returned 404. This supersedes the earlier note that production still served old frontend assets.

The Python contract suite passes 41/41; TypeScript typecheck and Vite build pass. These checks plus the asset match establish the current frontend bundle is deployed, but do not establish PHP/PostgreSQL transaction behavior, migration state, realtime recovery, or authenticated business flows. The computer-use inventory still reports no available apps or browsers, so no login, table mutation, second-participant privacy check, or live action audit was performed. The business audit remains **partial and unaccepted** pending exact-release authenticated multi-user/runtime acceptance.
