...
The browser owns one realtime connection for the currently opened session. A view refresh or rerender after an HTTP mutation or `session_changed` notification must not request a new ticket or replace a healthy socket. Opening another session or returning to the home view closes and invalidates the old connection and cancels its pending retry. Socket callbacks and retry timers must ignore superseded connection generations. A `session_changed` message refreshes the authorized HTTP snapshot while preserving the current socket. The UI reports “Connected” only after the current socket opens; HTTP snapshot success alone is not realtime connectivity. A current-socket disconnect schedules at most one backoff retry, and a successful open resets the retry delay.
